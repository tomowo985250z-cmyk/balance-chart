...
(async () => {
  ...
  try {
    ...
    near(engine.inspect().models['LINK:red'].angleCorrection, 3, 'first prospective angle correction at 0.30');
    ...
    near(learned.angle, 6.57, 'correction reaches next prediction');
    assert(learned.model === 'LINK:red', 'sparse BLD model falls back to parent');
    ...
    near(circle.inspect().models['LINK:red'].angleCorrection, 0.6, 'circular error learns +2, not -358');
    ...
    const distanceBeforeOutlier = circle.inspect().models['LINK:red'].distanceCorrection;
    ...
    assert(circle.inspect().models['LINK:red'].distanceCorrection <= distanceBeforeOutlier * 1.3 + 1e-8, 'outlier distance cap');
    ...
    assert(detail.inspect().models['LINK:red'].sampleCount === countBeforeEdit, 'edit replaces learning sample');
    detailSets[1].adjustments.push(['2', 'TAB', '1', 'DOWN']);
    detail.sync(detailSets, rotations);
    assert(detail.inspect().models['LINK:red'].sampleCount === countBeforeEdit - 1, 'mixed adjustment excluded');
    ...
    // 中心から遠ざかる候補しかない場合は採用しない。
    run("dotSets.at(-1).red=testDot(397,520,'red'); dotSets.at(-1).blue=testDot(397,520,'blue'); renderDots();");
    assert(run('guidePredictionDebug.reason') === 'no-improving-candidate', 'rejects worsening endpoints');
    assert(run("dotOverlay.querySelectorAll('.guide-direction-line').length") === 0, 'does not replace rejected prediction with worsening fallback');
