...
const BalanceLearning = (() => {
  ...
  function create({ storage, coordinates, nominalAngle, radius = 240 }) {
    const empty = () => ({ version: VERSION, models: {}, samples: [], pending: {}, manual: {}, order: [], blocked: [], excluded: [] });
    let state = empty();
    let signature = '';
    let storageError = null;
    try {
      const saved = JSON.parse(storage?.getItem(STORAGE_KEY) || 'null');
      if (saved?.version === VERSION && Array.isArray(saved.samples) && Array.isArray(saved.order)
    ...
    const stamp = set => JSON.stringify([set.red, set.blue, set.adjustments]);
    ...
    function train(sample, detailed) {
      ...
      if (actual.distance >= shortMovement) {
        ...
        // 循環誤差を制限し、最大でも1件あたり13.5度だけ更新する。
        const angleRate = RATE * Math.min(1, actual.distance / (radius * 0.1));
        model.angleCorrection = wrap(model.angleCorrection + clamp(error, -45, 45) * angleRate);
      }
      ...
      model.distanceCorrection = clamp(model.distanceCorrection * (1 + RATE * (ratio - 1)), 0.25, 4);
      model.sampleCount += 1;
    ...
    return { sync, predict, reset, recordManual,
