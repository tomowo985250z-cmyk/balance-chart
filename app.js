...
function renderDirectionLines() {
  ...
  const candidates = redDots.flatMap((red) => blueDots
    ...
    .sort((first, second) => Number(second.withinCenterThreshold) - Number(first.withinCenterThreshold)
  ...
  const fallbackRed = redDots.filter(({ line }) => line)
    .sort((first, second) => first.line.centerDistance - second.line.centerDistance || first.index - second.index)[0];
  const fallbackBlue = blueDots.filter(({ line }) => line)
    .sort((first, second) => first.line.centerDistance - second.line.centerDistance || first.index - second.index)[0];
  const selected = candidates[0] ?? (fallbackRed?.line && fallbackBlue?.line
    ? { redLine: fallbackRed.line, blueLine: fallbackBlue.line }
    : null);
  if (!selected) return;
