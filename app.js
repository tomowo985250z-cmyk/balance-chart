...
function renderTimeWheel(wheel, maximum, selected, setSelected) {
  ...
  for (let value = 0; value <= maximum; value += 1) {
      ...
      option.addEventListener('click', () => {
        setSelected(value);
        renderTimeWheel(wheel, maximum, value, setSelected);
      });
      wheel.append(option);
      if (value === selected) {
...
const redHourTrigger = populateTimeSelect(redInputs[0], 12, '時');
const redMinuteTrigger = populateTimeSelect(redInputs[1], 59, '分');
const blueHourTrigger = populateTimeSelect(blueInputs[0], 12, '時');
const blueMinuteTrigger = populateTimeSelect(blueInputs[1], 59, '分');
...
[redHourTrigger, redMinuteTrigger].forEach((trigger) => {
  trigger.addEventListener('click', () => openTimePicker(redInputs[0], redInputs[1]));
});
[blueHourTrigger, blueMinuteTrigger].forEach((trigger) => {
  trigger.addEventListener('click', () => openTimePicker(blueInputs[0], blueInputs[1]));
});
