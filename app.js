...
dotForm.addEventListener('submit', (event) => {
  ...
  const syncDotOverlayViewBox = () => {
    dotOverlay.setAttribute('viewBox', svg.getAttribute('viewBox') || '0 0 794 1123');
  };
  syncDotOverlayViewBox();
  new MutationObserver(syncDotOverlayViewBox).observe(svg, { attributes: true, attributeFilter: ['viewBox'] });
  redInputs[0].focus();
});
...
function setupRotationControls() {
  ...
  groups.forEach((state) => {
    state.handle.addEventListener('pointerdown', (event) => {
      if (rotationLocked || rotationLock.checked || svg.dataset.pinching === 'true') {
